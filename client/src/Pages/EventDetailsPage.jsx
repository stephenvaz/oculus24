  ...
  return (
    <div className='w-full min-h-[screen] relative pb-12'>
        ...
        <div className='x-container'>
          <div className='flex flex-col items-center justify-center lg:flex-row lg:justify-between'>
            <div className='flex flex-row gap-2 items-center'>
              <img src="/hackathon.png" alt="hackathon logo" className="w-32 lg:w-48" />
              <div className='flex flex-col items-center lg:items-start'>
                <h1 className='text-2xl lg:text-4xl font-bold text-white'>{event.title}</h1>
                <p className='text-xl lg:text-2xl text-white text-center'>{event.category} • {event.eventDate.month} {event.eventDate.year}</p>
              </div>
            </div>
      ...
      <div className='x-container mt-5 lg:mt-0'>
        ...
        <div className='w-full text-slate-100 text-md font-semibold'>
          ...
          {selected == "Rules" ? (
            <ul>
              {event.rules.map((rule, index) => {
                return <li key={index} className='py-1 flex flex-row items-center'><svg
                  aria-hidden="true"
                  focusable="false"
                  data-prefix="fas"
                  data-icon="circle-check"
                  className={`svg-inline--fa fa-circle-check mr-2`}
                  style={{ color: event.color, width: "20px", height: "20px" }}
                  role="img"
